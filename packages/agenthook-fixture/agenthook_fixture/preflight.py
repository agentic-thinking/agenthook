...
def run_preflight(client: CollectorClient | None = None, session_id: str | None = None) -> dict[str, Any]:
    ...
    emit("SessionStart", metadata={"runtime": "agenthook-fixture", "preflight": True, "spec_version": "0.2"})
    prompt = "AgentHook flight test: exercise all standard hooks"
    emit(
        "UserPromptSubmit",
        metadata={"prompt": prompt, "prompt_chars": len(prompt), "blocking": True, "preflight": True},
    )
    emit(
        "PreLLMCall",
        tool_name="llm.chat",
