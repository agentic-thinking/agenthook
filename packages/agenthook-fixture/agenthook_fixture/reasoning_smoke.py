...
def run_reasoning_smoke(prompt: str, client: CollectorClient | None = None) -> dict[str, Any]:
    ...
    collector.emit("SessionStart", sid, metadata={"runtime": "agenthook-fixture", "reasoning_smoke": True, "spec_version": "0.2"})
    collector.emit(
        "UserPromptSubmit",
        sid,
        metadata={"prompt": prompt, "prompt_chars": len(prompt), "blocking": True, "reasoning_smoke": True},
    )
    collector.emit(
        "PreLLMCall",
        sid,
    ...
    data = _post_json(
        ...
        payload,
        {
            ...
            "User-Agent": os.getenv("LLM_USER_AGENT", "agenthook-agenthook-fixture/0.1"),
        },
    )
